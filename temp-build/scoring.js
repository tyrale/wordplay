 ...
 */
export function analyzeWordChange(previousWord, currentWord, keyLetters = []) {
    // Normalize inputs
 ...
 */
export function calculateScore(previousWord, currentWord, options = {}) {
    ...
    if (!analysis.isRearranged && (analysis.addedLetters.length > 0 || analysis.removedLetters.length > 0)) {
        ...
        if (prevSorted !== currSorted) {
            ...
            if (prev.length === curr.length && analysis.addedLetters.length > 0 && analysis.removedLetters.length > 0) {
                ...
                const stayedLetters = prevChars.filter(char => !analysis.removedLetters.includes(char) && currChars.includes(char));
 ...
 */
export function getScoreForMove(previousWord, currentWord, keyLetters = []) {
 ...
 */
export function validateScoringResult(result) {
 ...
 */
export function performanceTestScoring(iterations = 1000) {
    const testMoves = [
 ...
 */
export function formatScoreBreakdown(result) {
    const parts = [];
